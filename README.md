...
I did lots of research online and tried out so many different stuffs myself and finally successfully lost 100 lbs. in a year and began my fitness journey ever since. I got my personal trainer certificate from American Council on Exercise (ACE), and I am a certified nutritional specialist from ACE as well. I learned how to be fit and stay fit. That’s when I started to monitor my body weight constantly. I lost a year or two of body weight data initially because I was using another app.   
...
The reason I got the idea of this project is that I have just started my yearly routine cutting phase. This is because every year I go on a cycle of gaining a few pounds during the winter and holiday seasons and start a new cutting phase the next year around the end of winter or start of spring to be back in six-pack shape. During this cutting phase, I count calories and monitor my body weight very closely to understand my caloric deficit state. And because there are no apps out there that can do what I needed to do, which is a weekly average body weight analysis. I must manually calculate my weekly average every single week to understand my current cutting situations. The reason weekly average is so important in weight management is daily body weight fluctuates too much for analysis, there are numerous reasons that can cause dramatic changes in body weight daily, such as meal timing, sleep, stress level, time of record, etc. These can cause unnecessary attention to unimportant trends. Weekly averages on the other hand, make so much more sense. It is so much easier to see the overall trends with weekly averages and monitor how someone is doing when it comes to body weight management. That’s where I got the idea of automating the entire process and using even more machine learning tools to analyze underlying trends by building a web application.

(1kg of pure body fat is roughly 7700 Kcal (Calories), so on a weekly average base, during the cutting phase, I can easily know what my daily caloric deficit is, which I can further determine if I need to adjust my daily diet.)
...
## Objectives
The main objective for this project is to build a comprehensive body weight analysis tool for myself to use which can automate my daily manual routine. To do that, I need the application to have the following functions:

- Record my body weight daily and upload it to a cloud-based SQL database where it stores all my body weight data.
- Because I need to record the weight on the page, I can’t risk people messing up my database, so I must add a password function so that only by entering the correct password, one can record body weight and perform other operations such as train the model.
...
- As a data scientist, I want to implement machine learning tools to do some seasonality analysis and discover underlying trends for me, as well as predictions just for fun. (I care about seasonality analysis, predictions on the other hand are not that useful for me in body weight analysis, they are just for fun)
...
1. **Basic Information**
    - This part contains basic information such as weight tracking information, weekly summaries, date, and so on.
...
1. **Weight Data Plots**
    ...
    - It also contains all my weight data displayed on different metrics, such as daily, weekly, monthly and seasonally averages.
1. **Machine Learning Insights**
    - This part contains all the information about the Facebook prophet model.
...
2. **Create a Cloud Based SQL Database** 
    ...
    - I tried to use SQLite first since that was what I am most familiar with, unfortunately, it was a dead end since I can’t put the database on cloud for Heroku deployment.
...
3. **Building Dash Application**
    - Created a virtual environment to isolate the project and only install necessary  packages.
    ...
    - Callback functions
        - For Dash, I need to create so many callback functions to get the desired results.
    - Feature design
...
4. **Modeling and Adjusting Database Along the side on Jupyter Notebook**
    ...
    - Saving model to cloud database
        ...
        - Although it is not ideal, since the trained model is small, I was able to serialize it and save it into a column of a newly created table in my cloud-based PostgreSQL database.
        - Now with this being done, I can load the model from my main .py file from the database.
    - Database fixes and adjustments
        - While building the application, I need to keep testing how it works, and some of the things I did always messes up the database. So, I must keep fixing the part I messed up and checking to make sure the database is running at its best.

5. **Deployment**
    - Create the requirements.txt, runtime.txt, Procfile, files for Heroku deployment.
    ...
    - Adjust time zone that the app is running in to my time zone on Heroku, since this application is very time sensitive. Queries all rely on relative time to run.
...
- For weight data plots, I can easily compare my daily weight and weekly average on the same time span to see differences. Notice that weekly averages are so important when it comes to weight management since on a daily scale, weight fluctuates too much to see any clear trends, especially in a short period of time! You can easily see what I am talking about by looking at the picture below. I also have the option to see my overall data on different average scales to know my status.

<img src="https://raw.githubusercontent.com/DericMei/First-Trial/main/weight%20data%20plots.png" width="600" height="250"/>

- For the Facebook prophet model, I can easily see my weekly and yearly seasonality trends. If you look at the plot below, on a weekly basis over the past years, I normally will go on some wild cheat meals during the weekends and goes back to clean diet on weekdays, that is why you can see that my weight normally is the highest for Saturdays and Mondays, and lowest on Wednesdays. For yearly trends, my body weight normally will peak at the beginning of February because it is the holiday seasons ending with the Chinese New Year!
...
## Learnings/Obstacles
There are so many stuffs I did not know, I struggled so much going through with this project since I had to learn so many new stuff along the way to make everything work. Here are some major learnings:
...
1. **Environmental Variables**
    ...
    - After receiving the automated Heroku credential leak message, I quickly learned that I need to setup environmental variables to store sensitive information.
    - I learned how to set up both local environmental variables for development and with Heroku on the cloud.
1. **Dash**
    ...
    - Now I am confident with Dash already that I am going to build more Dash apps in the future.
1. **HTML**
